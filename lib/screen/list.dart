...
class _ResultScreenState extends State<ResultScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('PrivateRoom').snapshots(includeMetadataChanges: true), // 데이터 실시간으로 받기
              ...
              return Container(
                ...
                child: ElevatedButton(
                  child: Column(
                    children: [
                      ListTile(
                        title: Text("방제목 : " + data["roomName"], // 방제목 쿼리
                          style: TextStyle(
                  ...
                  onPressed: () {
                    ...
                    FirebaseFirestore.instance.collection("Users").doc(user?.uid).get().then((DocumentSnapshot ds){
                      ...
                      FirebaseFirestore.instance.collection("PrivateRoom").doc(document.id).update({"Users" : FieldValue.arrayUnion([username])}); // 방 클릭시 유저 배열에 업데이트하기
                    });

                    for(var j = 0; j < userlist.length; j++) // 유저리스트를 다 읽어서 내가 그 리스트에 있으면 방 안에 있다고 인식!
                        {
                    ...
                    if(start != true) // 게임이 시작했는가?
                      ...
                      if(count <= 10) // 카운터 10 미만이면
                          {
                        FirebaseFirestore.instance.collection("PrivateRoom").doc(document.id).update({"Count" : count}); // 카운터를 올린다
                        Navigator.push(  // 게임 화면으로 이동
                          context,
                          MaterialPageRoute(
                      ...
                      if(roomIn == true) // 사용자가 들어가있는 상태에서 10명이 다 차면
                          {
                        Navigator.push(context, MaterialPageRoute(builder: (BuildContext)=> SubPage())); // 바로 서브페이지로 이동(미션 스킵)
                      }
                      else {
