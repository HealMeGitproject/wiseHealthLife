...
class MyCustomGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(               //Streambuilder를 이용하여 파이어베이스에서 스트림의 최신값을 자동으로 가져와 자동으로 UI를 업데이트 함
          stream:
          FirebaseFirestore.instance.collection('그래프').snapshots(includeMetadataChanges: true),
          builder: (BuildContext context, AsyncSnapshot<QuerySnapshot>snapshot){
            ...
            return Container(
              ...
              child: Center(
                child: CustomPaint(
                  size: Size(300, 300),
                  foregroundPainter: BarChart(                   //그래프 파이어베이스에서 값을 받아와서 반영하여 막대그래프의 수치로 표현해야하나 아직 받아오고나서 표현하는것이 제대로 구현 안됨
...
class BarChart extends CustomPainter {
  ...
  void drawBar(Canvas canvas, Size size, List<Offset> coordinates) {   //막대그래프 넓이위치설정
    Paint paint = Paint()
      ..color = color
  ...
  void drawXLabels(Canvas canvas, Size size, List<Offset> coordinates){
    ...
    for (int index = 0; index < labels.length; index++){
      TextSpan span = TextSpan(                         //막대 그래프 안 숫자의 크기, 색깔 등,,,
        style: TextStyle(
          color: Colors.white,
          fontSize: fontSize,
          fontFamily:  'Roboto',
