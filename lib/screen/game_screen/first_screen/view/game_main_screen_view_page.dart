...
class _StartPageState extends State<StartPage> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<DocumentSnapshot<Object?>>( // 데이터를 받아옴
      future: room.doc().get(), // 문제의 '그' 코드 여러가지 문서에 드나들 수 있도록 문서 ID를 받아야하는데 ID를 받는 방법을 모르겠습니다.
      builder:
          (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
        Map<String, dynamic> data =
        snapshot.data!.data() as Map<String, dynamic>; // 여러 데이터들을 분리하여 받아내는...
        var id = data['id'];
        print(id);
        ...
        return Scaffold( // 본 메인 화면
          ...
          appBar: AppBar(
            elevation: 0.0,
            title: Text('${data['roomName']} ${data['Count']}/10', // 이름과 인원 수...
                style: TextStyle(
